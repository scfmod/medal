...
impl<'a> Lifter<'a> {
    ...
    fn lift_function(&mut self) {
        ...
        // Populate register debug info on Function for PC-aware naming during SSA construction.
        let bytecode_func = &self.function_list[self.function.id];
        for info in &bytecode_func.local_debug_info {
            if info.name_index > 0 {
                if let Some(name_bytes) = self.string_table.get(info.name_index - 1) {
                    if let Ok(name) = String::from_utf8(name_bytes.clone()) {
                        self.function.register_debug_info.push(
                            cfg::function::RegisterDebugInfo {
                                register: info.register,
                                name,
                                scope_start: info.scope_start,
        ...
        // Populate statement_pc mapping and local_to_register during block lifting.
        for (start_pc, end_pc) in block_ranges {
            let node = self.block_to_node(start_pc);
            self.current_node = Some(node);
            let (statements, statement_pcs, edges) = self.lift_block(start_pc, end_pc);
            // Record actual bytecode PC for each statement in this block.
            for (stat_index, &pc) in statement_pcs.iter().enumerate() {
                self.function.statement_pc.insert((node, stat_index), pc);
            }
        ...
        // Populate local_to_register from the register map.
        for (&(reg_index, _), local) in &self.register_map {
            self.function
                .local_to_register
                .insert(local.clone(), reg_index as u8);
        }
    ...
    fn lift_block(
        ...
        block_start: usize,
        block_end: usize,
    ) -> (Vec<ast::Statement>, Vec<usize>, Vec<(NodeIndex, BlockEdge)>) {
        let mut statements = Vec::with_capacity((block_start..=block_end).count());
        let mut statement_pcs = Vec::new();
        ...
        while let Some((index, instruction)) = iter.next() {
            ...
            let stmts_before = statements.len();
            match *instruction {
                Instruction::BC {
                    op_code,
                    a,
            ...
            // Record the PC for any statements generated by this instruction.
            for _ in stmts_before..statements.len() {
                statement_pcs.push(self.current_pc);
            }
        }
        ...
        (statements, statement_pcs, edges)
    }
